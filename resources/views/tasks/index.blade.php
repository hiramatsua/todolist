        ...
        <div class="col col-md-12">
            ...
            <form action="" method="get">
                ...
                <div class="form-group">
                    ...
                    <div class="d-flex">
                        <input type="text" name="search" class="form-control col-6" placeholder="タスク名で検索" value="{{ old('search') }}">
                        &nbsp;
                        <button type="submit" class="btn btn-light">
                            <i class="fas fa-search"></i>&nbsp;検索する</button>
                    </div>
                </div>    
            </form>
            <!-- タスク一覧 -->
                        ...
                        <tbody>
                            @foreach($tasks as $task)
                                <tr>
                                    ...
                                    <td>{{ $task->contents }}</td>
                                    <td class="center">
                                        <span class="{{ $task->status_class }}"></span></td>
                                    <td>{{ $task->due_date }}</td>
                                    <td>{{ $task->updated_at }}</td>
                                    <td>
                                        <a class="btn btn-light btn-sm" href="{{ route('tasks.edit',['id' => $task->category_id, 'task_id' => $task->id]) }}">
